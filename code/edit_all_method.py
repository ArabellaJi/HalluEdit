...
if __name__ == "__main__":
    ...
    for editing_method in editing_methods[:]:
        ...
        device_eval = 'cpu' if args.device_eval == -1 else f'cuda:{args.device_eval}'
        device_edit = 'cpu' if args.device_edit == -1 else f'cuda:{args.device_edit}'

        # Also update the hparams device
        hparams.device = device_edit
        edit_kwargs = {
            'subject': subjects,
            'prompts': questions,
            'target_new': targets,
