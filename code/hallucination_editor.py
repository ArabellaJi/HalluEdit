...
class BaseEditor:
    ...
    def __init__(self, hparams: HyperParams):
        ...
        if not hparams.model_parallel and hasattr(hparams, 'device'):
            if hparams.device == -1 or str(hparams.device).lower() == 'cpu':
                self.model.to('cpu')
            else:
    ...
    def edit(self,
        ...
        # if device_eval == 'cuda:-1' or device_eval == -1:
        #     device_eval = 'cpu'
        # Handle CPU device properly
        if device_eval == 'cuda:-1' or device_eval == -1 or str(device_eval).lower() == 'cpu':
            device_eval = 'cpu'
            torch_dtype = torch.float32  # Use float32 for CPU
        else:
            torch_dtype = 'auto'

        model_eval = AutoModelForCausalLM.from_pretrained(eval_model_id, torch_dtype=torch_dtype).to(device_eval)
